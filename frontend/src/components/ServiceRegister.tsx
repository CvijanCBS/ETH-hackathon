...
export default function ServiceRegister() {
  ...
  const handleSubmit = (e: any) => {
    ...
    if (formData?.name.length < 2) return setFormError((prev) => ({ ...prev, name: "Name must be at least 2 characters long" }))
    ...
    // @ts-ignore
    if (formData?.car_brands.length < 1) return setFormError((prev) => ({...prev, car_brands: 'Select at least one car brand'}))
  ...
  const handleCheckBoxChange = async (e: any) => {
    ...
    // @ts-ignore
    if (carBrands.includes(e.target.name)) {
        // @ts-ignore
      carBrands.splice(carBrands.indexOf(e.target.name), 1)
    } else {
// @ts-ignore
        carBrands.push(e.target.name)
    }
    ...
    >
     ...
     <div>
        ...
        <div className="flex items-center me-4">
          <input
            checked={formData?.car_brands?.some((element) => element === 'AUDI') || false}
            type="checkbox"
            name="AUDI"
        ...
        <div className="flex items-center me-4">
          <input
            checked={formData?.car_brands?.some((element) => element === 'FIAT') || false}
            type="checkbox"
            name="FIAT"
            onChange={(e) => handleCheckBoxChange(e)}
            className="w-4 h-4 text-red-600 bg-gray-100 border-gray-300 rounded focus:ring-red-500"
        ...
        <div className="flex items-center me-4">
          <input
            checked={formData?.car_brands?.some((element) => element === 'RENAULT') || false}
            type="checkbox"
            name="RENAULT"
            onChange={(e) => handleCheckBoxChange(e)}
            className="w-4 h-4 text-red-600 bg-gray-100 border-gray-300 rounded focus:ring-red-500"
        ...
        <div className="flex items-center me-4">
          <input
            checked={formData?.car_brands?.some((element) => element === 'PAGANI') || false}
            type="checkbox"
            name="PAGANI"
            onChange={(e) => handleCheckBoxChange(e)}
            className="w-4 h-4 text-red-600 bg-gray-100 border-gray-300 rounded focus:ring-red-500"
        ...
        <div className="flex items-center me-4">
          <input
            checked={formData?.car_brands?.some((element) => element === 'KOENIGSEGG') || false}
            type="checkbox"
            name="KOENIGSEGG"
            onChange={(e) => handleCheckBoxChange(e)}
            className="w-4 h-4 text-red-600 bg-gray-100 border-gray-300 rounded focus:ring-red-500"
      ...
      >
        {isPending ? <Spinner className={"w-6 h-6"} /> : `Register shop`}
      </button>
    </form>
  )
}
